    ...
    public class OperationsWithFiles:ListFiles
    {
        ...
        public static void PasteAfterCopyFile(ListBox listItems)// вставка файла, с возможностью замены
        {
            try
            {
                ...
                {
                    if (File.Exists(ListFiles.thisName))
                    {
                        thisPath = ListFiles.thisName;
                        pathPaste = Path.Combine(varListPath, Path.GetFileName(thisPath));
                    }
                    else
                    {
                        //thisPath = Path.Combine(varListPath, ListFiles.thisName);
                        pathPaste = Path.Combine(varListPath, Path.GetFileName(thisPath));
                    }
